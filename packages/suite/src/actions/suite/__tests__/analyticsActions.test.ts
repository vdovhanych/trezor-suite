...
describe('Analytics Actions', () => {
    ...
    it('analyticsActions.report() - should report if enabled (desktop)', () => {
        ...
        expect(global.fetch).toHaveBeenNthCalledWith(
            1,
            'https://data.trezor.io/suite/log/desktop/beta.log?c_v=1.9&c_type=switch-device%2Feject&c_instance_id=1&c_session_id=very-random',
            { method: 'GET' },
        );
    ...
    it('analyticsActions.report() - should report if enabled (web)', () => {
        ...
        expect(global.fetch).toHaveBeenNthCalledWith(
            1,
            'https://data.trezor.io/suite/log/web/develop.log?c_v=1.9&c_type=switch-device%2Feject&c_instance_id=1&c_session_id=very-random',
            { method: 'GET' },
        );
        process.env.SUITE_TYPE = env;
    });
